...
#     section_needs_table,
# )
# from backend.services.text_utils import format_answers, sanitize_answers
...
# def _build_table_example(section_name: str) -> str:
#     lower = section_name.lower()

#     if any(k in lower for k in ["salary", "compensation", "ctc", "earnings", "gross", "payroll"]):
#         return (
#             f"TABLE EXAMPLE for '{section_name}' (salary/compensation type):\n"
#             '"content": [\n'
#             '  {"cells": ["Component", "Monthly (INR)", "Annual (INR)"]},\n'
#             '  {"cells": ["Basic Salary", "<value from data>", "<value × 12>"]},\n'
#             '  {"cells": ["House Rent Allowance", "<value from data>", "<value × 12>"]},\n'
#             '  {"cells": ["Total CTC", "--", "<total from data>"]}\n'
#             "]\n"
#             "Use actual values from the variable data above.\n"
#         )

#     if any(k in lower for k in ["invoice", "pricing", "quotation", "bill", "purchase"]):
#         return (
#             f"TABLE EXAMPLE for '{section_name}' (invoice/pricing type):\n"
#             '"content": [\n'
#             '  {"cells": ["Description", "Qty", "Unit Price", "Total"]},\n'
#             '  {"cells": ["<item from data>", "<qty>", "<unit price>", "<total>"]},\n'
#             '  {"cells": ["Sub-Total", "", "", "<subtotal>"]},\n'
#             '  {"cells": ["GST (18%)", "", "", "<gst amount>"]},\n'
#             '  {"cells": ["Grand Total", "", "", "<grand total>"]}\n'
#             "]\n"
#             "Use actual values from the variable data above.\n"
#         )

#     if any(k in lower for k in ["risk", "finding", "vulnerability", "defect"]):
#         return (
#             f"TABLE EXAMPLE for '{section_name}' (risk/findings type):\n"
#             '"content": [\n'
#             '  {"cells": ["ID", "Finding", "Severity", "Affected Area", "Status"]},\n'
#             '  {"cells": ["F-01", "<finding from data>", "High", "<area>", "Open"]},\n'
#             '  {"cells": ["F-02", "<finding from data>", "Medium", "<area>", "In Progress"]}\n'
#             "]\n"
#             "Populate from the findings in the variable data.\n"
#         )

#     if any(k in lower for k in ["timeline", "schedule", "milestone", "roadmap"]):
#         return (
#             f"TABLE EXAMPLE for '{section_name}' (timeline/schedule type):\n"
#             '"content": [\n'
#             '  {"cells": ["Phase", "Activity", "Owner", "Start Date", "End Date", "Status"]},\n'
#             '  {"cells": ["Phase 1", "<activity>", "<owner>", "<start>", "<end>", "Planned"]},\n'
#             '  {"cells": ["Phase 2", "<activity>", "<owner>", "<start>", "<end>", "Planned"]}\n'
#             "]\n"
#             "Use dates and activities from the variable data.\n"
#         )

#     if any(k in lower for k in ["kpi", "metric", "comparison", "performance"]):
#         return (
#             f"TABLE EXAMPLE for '{section_name}' (metrics/KPI type):\n"
#             '"content": [\n'
#             '  {"cells": ["Metric", "Target", "Actual", "Variance", "Status"]},\n'
#             '  {"cells": ["<metric name>", "<target>", "<actual>", "<variance>", "On Track"]},\n'
#             '  {"cells": ["<metric name>", "<target>", "<actual>", "<variance>", "At Risk"]}\n'
#             "]\n"
#             "Use metric values from the variable data.\n"
#         )

#     if any(k in lower for k in ["budget", "cost", "expense", "financial"]):
#         return (
#             f"TABLE EXAMPLE for '{section_name}' (budget/cost type):\n"
#             '"content": [\n'
#             '  {"cells": ["Category", "Budgeted (INR)", "Actual (INR)", "Variance"]},\n'
#             '  {"cells": ["<category>", "<budget>", "<actual>", "<variance>"]},\n'
#             '  {"cells": ["Total", "<total budget>", "<total actual>", "<total variance>"]}\n'
#             "]\n"
#             "Use financial values from the variable data.\n"
#         )

#     # Generic fallback — still shows a clear structure without biasing content
#     return (
#         f"TABLE EXAMPLE for '{section_name}':\n"
#         '"content": [\n'
#         '  {"cells": ["Column A", "Column B", "Column C"]},\n'
#         '  {"cells": ["<value from data>", "<value from data>", "<value from data>"]},\n'
#         '  {"cells": ["<value from data>", "<value from data>", "<value from data>"]}\n'
#         "]\n"
#         "Choose column headers that best describe the data for this section.\n"
#         "Use actual values from the variable data above.\n"
#     )


# # ═══════════════════════════════════════════════════════
# # SECTION INTELLIGENCE BLOCK
# # ═══════════════════════════════════════════════════════
...
# Generate premium-quality, comprehensive, polished business documents — executive-ready for PDF/DOCX.
# Today's date: {today}

# ABSOLUTE RULES:
# 1. NEVER use [brackets] as placeholders — no [DATE], [NAME], [Company Name], [Amount]
# 2. Use exact values from the variable data
# 3. If a value is genuinely absent, use the role-appropriate default (e.g. "the Employee",
#    "the Authorised Signatory", "as per agreement") — never write bracket text
# 4. Use today's date {today} for any date not explicitly provided
# 5. No ## no ** no markdown symbols anywhere
# 6. No pipe | characters in text content (tables handled separately)

# SECTION WRITING RULES:
# - HEADER:          exact values only — date, names, reference, company, designation
# - OPENER/INTRO:    3-4 full paragraphs — purpose, context, scope, importance
# - BODY/OBLIGATION: minimum 300 words — detailed paragraphs
# - FINANCIAL:       ALWAYS a table with real column headers and real data rows
# - PROCESS:         numbered steps with role and action
# - CLOSURE:         specific actions with owner and timeline
# - SIGN_OFF:        formal block with actual names/designation — compact

# QUALITY RULES:
# - Use ALL provided variable data
# - No filler: "as mentioned above", "it is important to note"
# - Approval/signature blocks: never leave unfilled

# OUTPUT: Start directly. No preamble. End with sign-off block."""


# STRUCTURED_SYSTEM_PROMPT = """You are DocForge, a structured document generation engine for B2B SaaS.
# Output ONLY valid JSON — never plain text outside JSON values.
# Today's date: {today}

# PLACEHOLDER RULE (CRITICAL):
# - NEVER write [brackets] anywhere
# - Use exact values from variable data
# - Date fields: use {today} if not provided
# - Missing name: use role title (e.g. "the Employee", "the Authorised Signatory")
# - Missing amount: use "as per agreement"
# - Table cells MUST have real content

# SECTION TYPES:
# - HEADER:          content_type "text",  left,    50-150 words, exact values
# - OPENER/INTRO:    content_type "text",  justify, 180-350 words, 3-4 paragraphs
# - BODY/OBLIGATION: content_type "text",  justify, 300-700 words
# - FINANCIAL/TABLE: content_type "table", real header row + real data rows
# - DEFINITIONS:     content_type "list",  minimum 4 items
# - SIGN_OFF:        content_type "text",  center,  40-100 words
...
# ]
# Minimum 1 header + 1 data row. No | characters in cell values.

# STRICT OUTPUT RULES:
# 1. Return ONLY valid JSON
# 2. No ## no ** no markdown in any content string
# 3. All required sections must be present in stated order
# 4. No stub content — every section must be substantially written
# 5. word_count must be an integer"""
...
#     safe_answers         = sanitize_answers(answers)
#     section_intelligence = _build_section_intelligence(sections)
#     answers_formatted    = format_answers(safe_answers)

#     length_note = (
#         "COMPREHENSIVE: Every section must be fully written.\n"
#         "- BODY/OBLIGATION/EVIDENCE: minimum 400 words each\n"
#         "- OPENER/CLOSURE: minimum 200 words each\n"
#         "- HEADER/SIGN_OFF: compact and factual"
#         if is_long else
#         "PRECISE: Every section must be complete and professional.\n"
#         "- Content sections: minimum 250 words each\n"
#         "- HEADER/SIGN_OFF: compact and factual"
#     )

#     logger.debug("build_prompt | template=%r archetype=%s sections=%d", template_name, archetype, section_count)
...
# VARIABLE DATA — USE ALL VALUES:
# {answers_formatted}

# RULES:
# 1. Use every provided value — do not ignore any supplied data
# 2. Never use [brackets] — use real values or a role-appropriate default
# 3. Today's date is {today}
# 4. Table sections: build proper table with actual data values
# 5. No ##, no **, no markdown

# {length_note}

# Generate all {section_count} sections. Start immediately. End with sign-off block."""
...
#             "Apply the feedback. Keep all sections. Maintain professional tone."
#         )
#     return base
...
#         depth        = SECTION_DEPTH_HINT.get(role, "detailed and professional")
#         table_hint   = (
#             " [TABLE REQUIRED — real rows from variable data, min 1 header + data rows]"
#             if section_needs_table(section.section_name) else ""
#         )
...
#     answers_fmt = format_answers(safe_answers)

#     table_sections = [s for s in sections if section_needs_table(s.section_name)]
#     table_example_note = ""
#     if table_sections:
#         examples = [_build_table_example(s.section_name) for s in table_sections[:3]]
...
# REQUIRED SECTIONS — ALL {len(sections)} MUST BE PRESENT, IN ORDER:
# {sections_list}

# VARIABLE DATA — EMBED ALL APPLICABLE VALUES:
# {answers_fmt}

# PLACEHOLDER RULE (CRITICAL):
# - Never write [brackets] in the JSON output
# - Use exact values from variable data
# - Missing name → use role title ("the Employee", "the Authorised Signatory")
# - Missing amount → "as per agreement"
# - Table cells must always have real text
# {table_example_note}

# DEPTH REQUIREMENTS:
# - OPENER/BACKGROUND: minimum 180 words, 3-4 paragraphs
# - BODY/OBLIGATION: minimum 300 words, detailed
# - EVIDENCE: table + minimum 150 words analysis
# - HEADER: 50-150 words, exact values
# - SIGN_OFF: 40-100 words, formal block
...
#       "content_type": "text",
#       "content": "Fully written content — no brackets, no placeholders",
...
# For list sections: "content": ["Item one", "Item two", "Item three"]
# For table sections: "content": [{{"cells": ["H1","H2","H3"]}}, {{"cells": ["V1","V2","V3"]}}]
...
# - All content substantive and professionally written?
...
# ═══════════════════════════════════════════════════════
# DYNAMIC TABLE EXAMPLE  (replaces the static salary-only example)
# ═══════════════════════════════════════════════════════
...
def build_structured_prompt(
    ...
    # Per-section schema with depth hints
    sections_schema = []
    for i, section in enumerate(sections, 1):
        role         = classify_section_role(section.section_name)
        content_type = role_to_content_type(role, section.section_name)
