...
#         db=db,
#         template_id=request.document_type_id,
#         company=request.company,   # now a typed field, not getattr
#     )
...
#         structured_sections = structured.get("sections", [])
#     except Exception:
#         # Fallback: fetch sections for prompt, then plain-text generation
#         sections = crud.get_sections_by_template(db, request.template_id)
#         prompt = prompt_service.build_prompt(
#             sections=sections,
#             department_name=department.name,
#             template_name=template.name,
...
#             llm_service.generate_with_llm(
#                 prompt,
#                 system_prompt=prompt_service.DOCUMENT_SYSTEM_PROMPT.format(
...
#     company  = getattr(request, "company", None)
#     sections = crud.get_sections_by_template(db, session.template_id)
...
router = APIRouter()
logger = get_logger("docforge.routes.generate")
...
def generate_questions(request: GenerateQuestionsRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.error("generate_questions failed: %s", exc, exc_info=True)
...
def preview_document(request: PreviewDocumentRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.error("preview_document LLM error: %s", exc, exc_info=True)
        raise HTTPException(status_code=502, detail=f"LLM service error: {exc}")
    except Exception as exc:
        logger.error("preview_document unexpected error: %s", exc, exc_info=True)
...
def generate_document(request: GenerateDocumentRequest, db: Session = Depends(get_db)):
    ...
    session = crud.create_session(db=db, department_id=request.department_id, template_id=request.template_id)
    crud.update_session_status(db, session.id, "in_progress")
    ...
    try:
        ...
        logger.warning("generate_document: structured path failed (%s) — falling back to plain text", exc)
        sections = crud.get_sections_by_template(db, request.template_id)
        try:
            prompt = prompt_service.build_prompt(
                sections=sections,
            ...
            crud.update_session_status(db, session.id, "failed")
            logger.error("generate_document: fallback also failed: %s", fallback_exc, exc_info=True)
            raise HTTPException(status_code=500, detail=f"Document generation failed: {fallback_exc}")
...
def regenerate_document(request: RegenerateDocumentRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.warning("regenerate_document: structured path failed (%s) — using plain text fallback", exc)
        try:
            regen_prompt = prompt_service.build_regenerate_prompt(
                sections=sections,
                department_name=department.name,
            ...
            logger.error("regenerate_document: fallback failed: %s", fallback_exc, exc_info=True)
            raise HTTPException(status_code=500, detail=f"Failed to regenerate document: {fallback_exc}")
        structured_sections = []

    if not content:
        raise HTTPException(status_code=500, detail="Regeneration produced no content")
...
def regenerate_section(request: RegenerateSectionRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.error("regenerate_section LLM error: %s", exc, exc_info=True)
        raise HTTPException(status_code=502, detail=f"LLM service error: {exc}")
    except Exception as exc:
        logger.error("regenerate_section unexpected error: %s", exc, exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to regenerate section: {exc}")
    if "error" in result:
        raise HTTPException(status_code=404, detail=result["error"])
    return result
